...
void input(char* inputFile){
    ...
    while (1) {
        strcpy(fileType, inputFile + strlen(inputFile) - 4);
        if (strcmp(fileType, ".txt") == 0) {
            ...
            if (inputFilePointer != NULL) {
                ...
                printf("Can't open %s. Please enter a valid file name: ", inputFile);
                scanf("%s", inputFile);
            }
        } else {
...
void input(char* inputFile){
    ...
    scanf("%s", inputFile); 
    FILE *inputFilePointer; 
    while ((inputFilePointer=fopen(inputFile, "r")) == NULL){ //while a file does not exist in the system
        printf("Can't open %s.\nWhat file do you wish to read? ", inputFile);
        scanf("%s", inputFile);
    }
    fclose(inputFilePointer);
}
...
void output(char* outputFile){
    printf("Name the encoded text file, (.txt file ending advised): ");
    scanf("%s", outputFile); 
